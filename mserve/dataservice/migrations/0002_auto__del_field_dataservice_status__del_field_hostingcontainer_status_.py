...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'HostingContainer.status'
        db.delete_column('dataservice_hostingcontainer', 'status')

        # Deleting field 'MFile.created'
        db.delete_column('dataservice_mfile', 'created')

        # Adding field 'NamedBase.created'
        db.add_column('dataservice_namedbase', 'created', self.gf('django.db.models.fields.DateTimeField')(auto_now_add=True, default=datetime.date(2011, 11, 28), blank=True), keep_default=False)


    def backwards(self, orm):
        
        # User chose to not deal with backwards NULL issues for 'DataService.status'
        raise RuntimeError("Cannot reverse this migration. 'DataService.status' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'HostingContainer.status'
        raise RuntimeError("Cannot reverse this migration. 'HostingContainer.status' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'MFile.created'
        raise RuntimeError("Cannot reverse this migration. 'MFile.created' and its values cannot be restored.")

        # Deleting field 'NamedBase.created'
        db.delete_column('dataservice_namedbase', 'created')
