    ...
    def get_job_plots(types):
        ...
        for type in types:
            ...
            if type == "jobtype":
                ...
                count = now - datetime.timedelta(days=2)
                hour = datetime.timedelta(hours=1)
                prev = count - hour

                logging.info("from %s to : %s", count, now)

                taskplot = {}
                taskplot["options"] = { "type" : "time",
                            "size" : "wide",
                            "label" : "Last 24 Hours",
                            "xaxis": 
                                {
                                "mode" : "time",
                                "min" : time.mktime(count.timetuple())*1000,
                                "max" : time.mktime(now.timetuple())*1000
                                } }
                taskplot["data"] = []
                ...
                while count < now:
                    ...
                    prev = count
                    count = count + hour
                    taskplot["data"].append( [time.mktime(count.timetuple())*1000, str(len(tasks)) ])

                plots.append(taskplot)
                
            if type == "jobs":
                jobplot = {}
                jobplot["options"] = { "type" : "pie", "size" : "medium", "label" : "Job Types" }
                ...
                for taskname in set(taskstates.values_list("name",flat=True).distinct()):
                    subplot = {}
                    subplot["options"] = { "type" : "pie", "size" : "small", "label" : "Job %s" % taskname.split(".")[-1] }
                ...
                jobplot = {}
                jobplot["options"] = { "type" : "pie", "size" : "small", "label" : "Jobs "}
                jobplot["data"] = [
