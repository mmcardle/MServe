...
if [ "$MSERVE_ARCHIVE" == "NOTSET" ]; then
	...
	echo "Installing MSERVE from repository $mserve_url"
	...
	echo " $MSERVE_ARCHIVE"
	#tar -C $MSERVE_CHECKOUT -xvf $MSERVE_ARCHIVE #|| f_ "failed to untar MSERVE archive"
        tar -C $MSERVE_TEMP --extract --file=$MSERVE_ARCHIVE mserve || f_ "failed to untar MSERVE archive"
fi
